...
def check_and_notify():
    ...
    current_day = now.isoweekday()  # 1 = Monday
    current_day_thai = day_map[str(current_day)]
    current_time = now.strftime('%H:%M')

    lst = load_schedule()
    changed = False
    for event in lst:
        if event['day'] == current_day_thai and event['time'] == current_time and event['status'] == 'pending' and CHAT_ID:
...
def handle_message(msg):
    ...
    if text == '/start':
        ...
        try:
            ...
            add_schedule(day_map[day_num], t, m)
